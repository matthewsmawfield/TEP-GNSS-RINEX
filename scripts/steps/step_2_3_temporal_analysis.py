...
not a transient artifact. Within the TEP v0.7 (Jakarta) framework, temporal
stability probes whether the continuous spatial profile of the time field
(Temporal Topology) maintains its structure across independent annual epochs,
and whether the field gradient (Temporal Shear) driving the observable coupling
is a persistent feature of the geodetic reference frame rather than a
...
def run_temporal_analysis(csv_file, filter_name):
    ...
    binners = {
        m: {c: {p: _init_binner() for p in periods} for c in COHERENCE_TYPES}
        for m in METRICS
    }
    ...
    for chunk in pd.read_csv(csv_file, chunksize=10_000_000):
        if GOOD_STATIONS_SET:
            chunk = chunk[
                chunk["station1"].isin(GOOD_STATIONS_SET)
                & chunk["station2"].isin(GOOD_STATIONS_SET)
            ]
        ...
        for metric in METRICS:
            ...
            if np.any(valid_msc_all):
                _accumulate_binner(
                    binners[metric]["msc"]["all"],
                    bin_idx[valid_msc_all],
                    msc[valid_msc_all],
                )
            ...
            if phase is not None:
                ...
                if np.any(valid_phase_all):
                    _accumulate_binner(
                        binners[metric]["phase_alignment"]["all"],
                        bin_idx[valid_phase_all],
                        phase[valid_phase_all],
                    )
            ...
            for year in YEARS:
                ...
                if np.any(valid_msc):
                    _accumulate_binner(
                        binners[metric]["msc"][year], bin_idx[valid_msc], msc[valid_msc]
                    )
                ...
                if phase is not None:
                    ...
                    if np.any(valid_phase):
                        _accumulate_binner(
                            binners[metric]["phase_alignment"][year],
                            bin_idx[valid_phase],
                            phase[valid_phase],
                        )
    ...
    for metric in METRICS:
        for coh_type in COHERENCE_TYPES:
            ...
            for period in periods:
                bc, bm, bn = _finalize_binner(
                    binners[metric][coh_type][period], bin_edges
                )
                result = fit_exponential_binned(bc, bm, bn) if bc is not None else None
                all_results[mode][str(period)] = result
                if result:
                    print_status(
                        f"{period:<12} {result['lambda_km']:<12.0f} {result['r_squared']:<10.3f} {result['n_pairs']:>12,}"
                    )
                    if period != "all":
            ...
            if len(lambdas) >= 2:
                ...
                if unconstrained_years:
                    print_status(
                        f"  → WARNING: Potentially unconstrained λ in years: {', '.join(unconstrained_years)} (large relative fit uncertainty)",
                        level="WARNING",
                    )
    ...
    print_status(
        f"{'Mode':<30} {'2022 λ':<10} {'2023 λ':<10} {'2024 λ':<10} {'CV%':<8}"
    )
    print_status("-" * 70)
    ...
    for metric in METRICS:
        for coh_type in COHERENCE_TYPES:
            ...
            row.append(f"{cv:.1f}")
            print_status(
                f"{row[0]:<30} {row[1]:<10} {row[2]:<10} {row[3]:<10} {row[4]:<8}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TEP-GNSS-RINEX Step 2.3b: Temporal Analysis"
    )
    parser.add_argument(
        "--filter",
        type=str,
    ...
    print_status(
        "STEP 2.3b: Temporal Stability Analysis (COMPREHENSIVE)", level="TITLE"
    )
    ...
    print_status(
        f"Processing: {len(filters)} filters x {len(PROCESSING_MODES)} modes x {len(METRICS)} metrics x {len(COHERENCE_TYPES)} coherence types"
    )
    ...
    for filter_name, filter_config in filters:
        ...
        with open(
            RESULTS_DIR / f"step_2_3_temporal_analysis_{filter_key}{run_tag}.json", "w"
        ) as f:
            json.dump(output, f, indent=2, default=str)
        print_status(
            f"  Saved: step_2_3_temporal_analysis_{filter_key}{run_tag}.json",
            level="SUCCESS",
        )
    ...
    summary = {
        "step": "2.3b",
        "name": "temporal_analysis_comprehensive",
        "timestamp": datetime.now().isoformat(),
    ...
    with open(
        RESULTS_DIR / f"step_2_3_temporal_analysis_summary{run_tag}.json", "w"
    ) as f:
        json.dump(summary, f, indent=2, default=str)

    print_status(
        f"COMPLETE: {len(filters)} filters x {len(PROCESSING_MODES)} modes",
        level="SUCCESS",
    )
