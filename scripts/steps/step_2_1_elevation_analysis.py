...
Under the TEP v0.7 continuous geometric screening framework, stations at
higher altitude sit in lower ambient atmospheric density, which implies
less flattening of the Temporal Topology and therefore stronger Temporal
Shear (field gradient). This predicts longer correlation lengths at
altitude if the density-driven gradient suppression mechanism is active.
...
logger = TEPLogger(
    name="step_2_1_elevation_analysis", level="INFO", log_file_path=LOG_FILE_PATH
)
...
def process_metric(metric):
    ...
    short_range = {
        c: {q: {"sum": 0.0, "count": 0} for q in QUINTILES} for c in COHERENCE_TYPES
    }
    ...
    q_map, q_stats, band_map = get_station_quintiles_lat_controlled(
        coords, lat_band_deg=LAT_BAND_DEG
    )

    for k, v in q_stats.items():
        if v.get("count", 0) > 0:
            print_status(
                f"  {k}: {v['min']:.0f}m - {v['max']:.0f}m ({v['count']} stations)"
            )
    ...
    for chunk in pd.read_csv(CSV_FILE, chunksize=chunk_size):
        ...
        for q in QUINTILES:
            ...
            if REQUIRE_SAME_LAT_BAND:
                mask = np.array(
                    [
                        qa == q and qb == q and ba == bb
                        for qa, qb, ba, bb in zip(q1, q2, b1, b2)
                    ]
                )
            else:
            ...
            if np.any(mask):
                for coh_type in COHERENCE_TYPES:
                    ...
                    if np.any(valid):
                        ...
                        if np.any(sr_valid):
                            short_range[coh_type][q]["sum"] += float(
                                np.nansum(vals[sr_valid])
                            )
    ...
    for coh_type in COHERENCE_TYPES:
        ...
        for q in QUINTILES:
            ...
            res["short_range_mean"] = (
                float(sr_sum / sr_count) if sr_count > 0 else np.nan
            )
            ...
            if res:
                if "lambda_km" in res and "r_squared" in res:
                    print_status(
                        f"    {q:<4}: λ={res['lambda_km']:.0f}km, R²={res['r_squared']:.2f}, N={res.get('n_pairs', 0):,}"
                    )
                else:
                    print_status(
                        f"    {q:<4}: Fit Failed (N={res.get('n_samples', 0):,})",
                        level="WARNING",
                    )
            else:
        ...
        altitudes = np.asarray(
            [q_stats.get(q, {}).get("mean", np.nan) for q in QUINTILES], dtype=float
        )
        lambdas = np.asarray(
            [results[coh_type][q].get("lambda_km", np.nan) for q in QUINTILES],
            dtype=float,
        )
        ...
        weights_n = np.asarray(
            [results[coh_type][q].get("n_pairs", 0) for q in QUINTILES], dtype=float
        )
        results[coh_type]["trend_weighted_n_pairs"] = _weighted_linregress(
            altitudes, lambdas, weights_n
        )
        ...
        results[coh_type]["trend_weighted_invvar"] = _weighted_linregress(
            altitudes, lambdas, weights_iv
        )

        results[coh_type]["trend_amplitude"] = _trend_from_quintiles(
            q_stats, results[coh_type], "amplitude", "amplitude"
        )
        results[coh_type]["trend_offset"] = _trend_from_quintiles(
            q_stats, results[coh_type], "offset", "offset"
        )
        results[coh_type]["trend_short_range_mean"] = _trend_from_quintiles(
            q_stats, results[coh_type], "short_range_mean", "short_range_mean"
        )
...
def plot_summary(final_results):
    ...
    fig.suptitle(
        "TEP Elevation Dependence (Correlation Length λ vs Altitude Quintile)",
        fontsize=16,
    )
    ...
    for i, metric in enumerate(METRICS):
        for j, coh in enumerate(COHERENCE_TYPES):
            ...
            if len(valid_idx) >= 3:
                ...
                p = np.poly1d(z)
                ax.plot(
                    x, p(x), "r--", alpha=0.5, label=f"Slope: {z[0]:.1f} km/quintile"
                )
                ax.legend()
...
def _run_analysis(filters):
    ...
    for filter_name, filter_config in filters:
        ...
        output_file = (
            RESULTS_DIR
            / f"step_2_1_elevation_analysis_{filter_key}{OUTPUT_SUFFIX}.json"
        )
    ...
    with open(
        RESULTS_DIR / f"step_2_1_elevation_analysis_summary{OUTPUT_SUFFIX}.json", "w"
    ) as f:
        json.dump(summary, f, indent=2)

    first_filter = list(all_filter_results.keys())[0] if all_filter_results else None
    if first_filter and "baseline" in all_filter_results[first_filter].get(
        "results_by_mode", {}
    ):
        plot_summary(all_filter_results[first_filter]["results_by_mode"]["baseline"])

    print_status(
        f"COMPLETE: {len(filters)} filters × {len(PROCESSING_MODES)} modes",
        level="SUCCESS",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TEP-GNSS-RINEX Step 2.1b: Elevation Analysis"
    )
    parser.add_argument(
        "--filter",
        type=str,
