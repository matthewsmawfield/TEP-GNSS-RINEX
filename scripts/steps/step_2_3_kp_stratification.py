...
Within the TEP v0.7 (Jakarta) continuous geometric screening framework,
geomagnetic storms modulate ambient ionospheric density, which in turn
affects the local flattening of the Temporal Topology. If the observed
correlation lengths were driven by density-dependent gradient suppression
(Temporal Shear modulation), storm conditions would be expected to produce
measurable changes in the effective coupling. Near-invariance across Kp
strata supports a non-electromagnetic origin.
...
PROCESSING_MODES = {
    "baseline": {"description": "GPS L1 only"},
    "ionofree": {"description": "Dual-frequency L1+L2 ionosphere-free"},
    "multi_gnss": {"description": "GPS+GLO+GAL+BDS"},
    "precise": {"description": "IGS Precise Orbits (L1+L2)"},
}
...
def fit_exponential_binned(bin_centers, bin_means, bin_counts):
    ...
    try:
        ...
        lam_err = (
            float(np.sqrt(pcov[1, 1]))
            if pcov.shape == (3, 3) and np.isfinite(pcov[1, 1])
            else float("nan")
        )
        lam_rel_err = (
            float(lam_err / lam) if lam > 0 and np.isfinite(lam_err) else float("nan")
        )
...
def load_kp_index():
    ...
    try:
        ...
        raise RuntimeError(
            f"CRITICAL: Failed to download real Kp data from GFZ Potsdam: {e}. "
    ...
    print_status(
        f"  Kp range: {min(kp_values):.2f} - {max(kp_values):.2f}, mean: {np.mean(kp_values):.2f}"
    )
...
def process_metric(csv_file, target_metric, kp_values, kp_thresholds, station_set=None):
    ...
    bin_edges = np.logspace(
        np.log10(MIN_DISTANCE_KM), np.log10(MAX_DISTANCE_KM), N_BINS + 1
    )
    ...
    for chunk in pd.read_csv(csv_file, chunksize=chunk_size):
        ...
        phase = (
            mc["phase_alignment"].to_numpy(dtype=np.float64, copy=False)
            if "phase_alignment" in mc.columns
            else None
        )

        year_idx = years - YEARS[0]
        valid_day = (
            (year_idx >= 0) & (year_idx < len(YEARS)) & (doys >= 1) & (doys <= 366)
        )
        ...
        if np.any(valid_msc):
            for thr in kp_thresholds:
                ...
                if np.any(q_idx):
                    _accumulate_binner(
                        binners_by_thr[thr]["msc"]["quiet"], bin_idx[q_idx], msc[q_idx]
                    )
                if np.any(s_idx):
                    _accumulate_binner(
                        binners_by_thr[thr]["msc"]["storm"], bin_idx[s_idx], msc[s_idx]
                    )
        ...
        if phase is not None:
            ...
            if np.any(valid_phase):
                for thr in kp_thresholds:
                    ...
                    if np.any(q_idx):
                        _accumulate_binner(
                            binners_by_thr[thr]["phase_alignment"]["quiet"],
                            bin_idx[q_idx],
                            phase[q_idx],
                        )
                    if np.any(s_idx):
                        _accumulate_binner(
                            binners_by_thr[thr]["phase_alignment"]["storm"],
                            bin_idx[s_idx],
                            phase[s_idx],
                        )
    ...
    for thr in kp_thresholds:
        for c in COHERENCE_TYPES:
            ...
            all_sum = (
                binners_by_thr[thr][c]["quiet"]["sum"]
                + binners_by_thr[thr][c]["storm"]["sum"]
            )
            all_count = (
                binners_by_thr[thr][c]["quiet"]["count"]
                + binners_by_thr[thr][c]["storm"]["count"]
            )
            ...
            print_status(
                f"  {mode} (Kp≥{thr:g}): Q={q_res['lambda_km'] if q_res else 'N/A'}, S={s_res['lambda_km'] if s_res else 'N/A'}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TEP-GNSS-RINEX Step 2.3: Kp Stratification"
    )
    parser.add_argument(
        "--filter",
        type=str,
    ...
    print_status(
        "STEP 2.3a: Kp Geomagnetic Stratification (COMPREHENSIVE)", level="TITLE"
    )
    ...
    if args.filter == "all":
        ...
        matching = [
            f for f in STATION_FILTERS if f[1] == args.filter or f[0] == args.filter
        ]
        filters = matching if matching else [("ALL_STATIONS", "none")]

    print_status(
        f"Processing: {len(filters)} filters x {len(PROCESSING_MODES)} modes x {len(METRICS)} metrics x {len(COHERENCE_TYPES)} coherence types x {len(kp_thresholds)} Kp thresholds"
    )
    ...
    for thr in kp_thresholds:
        ...
        print_status(
            f"  Threshold Kp≥{thr:g}: Quiet={q} ({100 * q / (q + s):.1f}%), Storm={s} ({100 * s / (q + s):.1f}%)"
        )
    ...
    for filter_name, filter_config in filters:
        ...
        for mode_name, mode_config in PROCESSING_MODES.items():
            ...
            if not csv_file.exists():
                print_status(
                    f"    WARNING: {csv_file.name} not found, skipping", level="WARNING"
                )
                continue

            mode_results_by_thr = {thr: {} for thr in kp_thresholds}
            for metric in METRICS:
                res_by_thr = process_metric(
                    csv_file, metric, kp_values, kp_thresholds, station_set=station_set
                )
                for thr in kp_thresholds:
            ...
            for thr in kp_thresholds:
                ...
                for key, val in mode_results_by_thr[thr].items():
                    if val.get("quiet") and val.get("storm"):
                        ...
                        print_status(
                            f"      {key}: Q={q_lam:.0f}km, S={s_lam:.0f}km, Δ={delta:+.1f}%"
                        )
        ...
        for thr in kp_thresholds:
            ...
            with open(
                RESULTS_DIR
                / f"step_2_3_kp_stratification_{filter_key}{thr_tag}{run_tag}.json",
                "w",
            ) as f:
                json.dump(output, f, indent=2, default=str)
            print_status(
                f"  Saved: step_2_3_kp_stratification_{filter_key}{thr_tag}{run_tag}.json",
                level="SUCCESS",
            )
            ...
            if abs(thr - KP_THRESHOLD) < 1e-9:
                with open(
                    RESULTS_DIR
                    / f"step_2_3_kp_stratification_{filter_key}{run_tag}.json",
                    "w",
                ) as f:
                    json.dump(output, f, indent=2, default=str)
                print_status(
                    f"  Saved: step_2_3_kp_stratification_{filter_key}{run_tag}.json",
                    level="SUCCESS",
                )
    ...
    for thr in kp_thresholds:
        ...
        with open(
            RESULTS_DIR / f"step_2_3_kp_stratification_summary{thr_tag}{run_tag}.json",
            "w",
        ) as f:
        ...
        if abs(thr - KP_THRESHOLD) < 1e-9:
            with open(
                RESULTS_DIR / f"step_2_3_kp_stratification_summary{run_tag}.json", "w"
            ) as f:
                json.dump(summary, f, indent=2, default=str)

    print_status(
        f"COMPLETE: {len(filters)} filters × {len(PROCESSING_MODES)} modes × {len(kp_thresholds)} thresholds",
        level="SUCCESS",
    )
