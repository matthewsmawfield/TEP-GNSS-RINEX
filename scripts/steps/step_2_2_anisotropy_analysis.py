...
# =============================================================================
# TEMPORAL ORBITAL TRACKING - CODE METHODOLOGY FOR E-W/N-S RATIO
# =============================================================================
# This implements the same temporal_orbital_tracking methodology as CODE/IGS/ESA
# to enable fair comparison of E-W/N-S anisotropy ratios.
# 
# Methodology:
# - 30-day windows (±15 days) at 10-day intervals throughout the year
# - Separate E-W and N-S λ fits for each window
# - Compute E-W/N-S ratio for each window
# - Average across all windows → mean_ew_ns_ratio
# - Correlate ratio with orbital speed
# =============================================================================

def calculate_earth_orbital_motion(day_of_year):
    """
    Calculate Earth's orbital parameters for a given day of year.
    Matches CODE methodology exactly.
    """
    T = 365.25  # Orbital period in days
    e = 0.0167  # Eccentricity
    perihelion_day = 3  # ~January 3
    
    M = 2 * np.pi * (day_of_year - perihelion_day) / T
    E = M + e * np.sin(M)
    ...
    AU_km = 1.496e8
    v = np.sqrt(GM_sun * (2 / (r * AU_km) - 1 / (a * AU_km)))
...
def fit_directional_lambda_for_tracking(distances, coherences):
    """
    Fit exponential decay to get correlation length λ for temporal tracking.
    Returns λ in km or None if fit fails.
    """
    ...
    valid = (bin_idx >= 0) & (bin_idx < N_BINS) & ~np.isnan(coherences)
    
    if np.sum(valid) < 100:
        return None
    
    bin_idx = bin_idx[valid]
    coh = coherences[valid]
    
    bin_sums = np.bincount(bin_idx, weights=coh, minlength=N_BINS)
    bin_counts = np.bincount(bin_idx, minlength=N_BINS)
    
    ok = bin_counts >= MIN_BIN_COUNT
    if np.sum(ok) < 5:
        return None
    
    bin_centers = (bin_edges[:-1] + bin_edges[1:])[ok] / 2
    bin_means = bin_sums[ok] / bin_counts[ok]
    weights = bin_counts[ok]
    
    def exp_decay_model(r, A, lam, C0):
        return A * np.exp(-r / lam) + C0
...
def run_temporal_orbital_tracking(pairs_data, coherence_key='phase_alignment'):
    """
    Run temporal orbital tracking analysis (CODE methodology).
    
    This computes E-W/N-S ratios using 30-day windows at 10-day intervals,
    enabling direct comparison with CODE's 2.72, IGS's 3.11, ESA's 2.73.
    
    Parameters:
    -----------
    pairs_data : dict
        Dictionary with 'dist', 'azimuth', 'doy', 'year', and coherence arrays
    coherence_key : str
        Key for coherence values ('phase_alignment' or 'coherence')
        
    Returns:
    --------
    dict with temporal tracking results and mean_ew_ns_ratio
    """
    print_status("\n" + "="*70, "INFO")
    print_status("TEMPORAL ORBITAL TRACKING (CODE METHODOLOGY)", "TITLE")
    print_status("="*70, "INFO")
    print_status("  Purpose: Compute day-by-day E-W/N-S ratios for fair comparison", "INFO")
    print_status("  Reference: CODE=2.72, IGS=3.11, ESA=2.73 (2.5yr processed)", "INFO")
    
    if not pairs_data or 'dist' not in pairs_data:
        return {'success': False, 'error': 'No pairs data'}
    
    # Check required columns
    required = ['dist', 'azimuth', 'doy']
    for col in required:
        if col not in pairs_data:
            return {'success': False, 'error': f'Missing column: {col}'}
    
    if coherence_key not in pairs_data:
        coherence_key = 'coherence' if 'coherence' in pairs_data else None
        if not coherence_key:
            return {'success': False, 'error': 'No coherence data'}
    
    distances = pairs_data['dist']
    azimuths = pairs_data['azimuth']
    doys = pairs_data['doy']
    coherences = pairs_data[coherence_key]
    
    # Classify E-W vs N-S (using 45° sectors like CODE)
    ew_mask = np.array([is_ew_azimuth(az) for az in azimuths])
    ns_mask = np.array([is_ns_azimuth(az) for az in azimuths])
    
    print_status(f"  Total pairs: {len(distances):,}", "INFO")
    print_status(f"  E-W pairs: {np.sum(ew_mask):,}", "INFO")
    print_status(f"  N-S pairs: {np.sum(ns_mask):,}", "INFO")
    print_status(f"  Coherence metric: {coherence_key}", "INFO")
    
    # Temporal tracking parameters (same as CODE)
    DAY_WINDOW = 15  # ±15 days = 30-day window
    DAY_SAMPLES = range(15, 351, 10)  # Every 10 days
    MIN_PAIRS = 500
    
    print_status(f"  Window: ±{DAY_WINDOW} days (30-day total)", "INFO")
    print_status(f"  Samples: {len(list(DAY_SAMPLES))} across year", "INFO")
    
    # Build DOY lookup for efficiency
    unique_doys = np.unique(doys)
    doy_indices = {d: np.where(doys == d)[0] for d in unique_doys}
    
    temporal_tracking = []
    
    for center_doy in DAY_SAMPLES:
        # Collect indices from ±DAY_WINDOW days
        window_indices = []
        for d in range(center_doy - DAY_WINDOW, center_doy + DAY_WINDOW + 1):
            lookup_d = d
            if d <= 0:
                lookup_d = 365 + d
            elif d > 365:
                lookup_d = d - 365
            if lookup_d in doy_indices:
                window_indices.extend(doy_indices[lookup_d].tolist())
        
        if len(window_indices) < 1000:
            continue
        
        window_indices = np.array(window_indices)
        
        # Get E-W and N-S subsets for this window
        ew_idx = window_indices[ew_mask[window_indices]]
        ns_idx = window_indices[ns_mask[window_indices]]
        
        if len(ew_idx) < MIN_PAIRS or len(ns_idx) < MIN_PAIRS:
            continue
        
        # Fit λ for E-W and N-S
        ew_lambda = fit_directional_lambda_for_tracking(
            distances[ew_idx], coherences[ew_idx]
        )
        ns_lambda = fit_directional_lambda_for_tracking(
            distances[ns_idx], coherences[ns_idx]
        )
        
        if ew_lambda is not None and ns_lambda is not None and ns_lambda > 0:
            ew_ns_ratio = ew_lambda / ns_lambda
            orbital_params = calculate_earth_orbital_motion(center_doy)
            
            temporal_tracking.append({
                'day_of_year': center_doy,
                'ew_lambda_km': ew_lambda,
                'ns_lambda_km': ns_lambda,
                'ew_ns_ratio': ew_ns_ratio,
                'n_ew_pairs': len(ew_idx),
                'n_ns_pairs': len(ns_idx),
    ...
    print_status(f"  Valid temporal samples: {len(temporal_tracking)}", "INFO")
    
    if len(temporal_tracking) < 10:
        return {'success': False, 'error': f'Insufficient samples: {len(temporal_tracking)}'}
    ...
    mean_ratio = float(np.mean(ew_ns_ratios))
    std_ratio = float(np.std(ew_ns_ratios))
    
    print_status("\n  " + "-"*50, "SUCCESS")
    print_status(f"  MEAN E-W/N-S RATIO: {mean_ratio:.3f} ± {std_ratio:.3f}", "SUCCESS")
    print_status(f"  Range: [{min(ew_ns_ratios):.2f}, {max(ew_ns_ratios):.2f}]", "INFO")
    print_status(f"  Orbital correlation: r={orbital_correlation:.3f}, p={orbital_p_value:.2e}", "INFO")
    print_status("  " + "-"*50, "SUCCESS")
    
    # Comparison
    print_status("\n  COMPARISON WITH PROCESSED DATA:", "INFO")
    print_status(f"    CODE (2.5yr):  2.72", "INFO")
    print_status(f"    IGS (2.5yr):   3.11", "INFO")
    print_status(f"    ESA (2.5yr):   2.73", "INFO")
    print_status(f"    CODE (25yr):   1.89", "INFO")
    print_status(f"    RINEX (this):  {mean_ratio:.2f}", "SUCCESS")
    ...
    return {
        ...
        'statistical_analysis': {
            ...
            'mean_ew_ns_ratio': mean_ratio,
            'ew_ns_ratio_std': std_ratio,
            'ew_ns_ratio_range': [float(min(ew_ns_ratios)), float(max(ew_ns_ratios))]
        },
        'methodology': {
            'window_days': DAY_WINDOW * 2,
            'sample_interval_days': 10,
            'min_pairs_per_direction': MIN_PAIRS,
            'coherence_metric': coherence_key
        },
        'comparison': {
            'code_2_5yr': 2.72,
            'igs_2_5yr': 3.11,
...
def run_analysis_pipeline(filter_mode_name, station_filter_config, observables=None):
    ...
    # ==========================================================================
    # TEMPORAL ORBITAL TRACKING (CODE METHODOLOGY) - For fair E-W/N-S comparison
    # ==========================================================================
    print_status("\n[5/8] Running Temporal Orbital Tracking (CODE methodology)...", "PROCESS")
    temporal_orbital_tracking_result = run_temporal_orbital_tracking(pairs_all, 'phase_alignment')
    
    # Also run on coherence metric for comparison
    temporal_orbital_tracking_coherence = run_temporal_orbital_tracking(pairs_all, 'coherence')
    ...
    output = {
        ...
        'temporal_orbital_tracking': {
            'phase_alignment': temporal_orbital_tracking_result,
            'coherence': temporal_orbital_tracking_coherence
        },
        'baseline': {
            'all_days': result_all,
    ...
    executive_summary = {
        ...
        'temporal_orbital_tracking': {
            'method': 'CODE methodology (30-day windows, 10-day intervals)',
            'mean_ew_ns_ratio': temporal_orbital_tracking_result.get('statistical_analysis', {}).get('mean_ew_ns_ratio') if temporal_orbital_tracking_result.get('success') else None,
            'comparison': {
                'code_2_5yr': 2.72,
                'igs_2_5yr': 3.11,
                'esa_2_5yr': 2.73,
